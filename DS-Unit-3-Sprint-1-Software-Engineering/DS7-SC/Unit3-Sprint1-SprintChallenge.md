...
_your_ work and be saved in `.py` files (_not_ notebooks), and (along with this
...
Write a Python `class` to model the above data. Make sure you are _precise_ in
...
_Hint_ - `random.randint` should be able to serve your random number needs.
...
The class you wrote in part 1 is nice, but it doesn't _do_ anything - that is,
it doesn't have any _methods_. So let's add some! Specifically, add two methods:
...
- `name` should be a random adjective from `['Awesome', 'Shiny', 'Impressive', 'Portable', 'Improved']` followed by a space and then a random noun from
...
- Add at least _2_ more test methods to `AcmeProductTests` for the base
...
_Hint_ - `test_legal_names` is the trickiest of these, but may not be as bad as
...
_return_ anything - it just prints (a "side-effect"). For the purposes of this
...
as well as _this_ file with your answers to part 7, to your weekly repo
...
_Bonus!_ Got this far? Read up on the [history of the fine Acme
